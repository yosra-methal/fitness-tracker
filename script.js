...
const defaultExercises = [
    { id: 'bench', name: 'Développé Couché', defaultSets: 4, defaultReps: 10, defaultWeight: 60 },
    { id: 'squat', name: 'Squat', defaultSets: 5, defaultReps: 5, defaultWeight: 80 },
    { id: 'pullup', name: 'Tractions', defaultSets: 4, defaultReps: 8, defaultWeight: 0 },
    { id: 'dl', name: 'Soulevé de Terre', defaultSets: 3, defaultReps: 5, defaultWeight: 100 }
];
...
function renderSelectionMode() {
    ...
    header.innerHTML = `<h2>Exercices</h2>`;
    ...
    defaultExercises.forEach(ex => {
        ...
        btn.className = 'btn exercise-item';
...
function renderActiveEffortMode() {
    ...
    header.innerHTML = `
        ...
        <div class="text-secondary" style="font-weight:600">${state.currentExercise.name}</div>
    ...
    setDisplay.innerHTML = `
        <div class="text-secondary" style="font-size: 14px; text-transform: uppercase; letter-spacing: 1px;">Série Actuelle</div>
    ...
    validateBtn.textContent = 'Valider la série';
...
function renderActiveRestMode() {
    ...
    header.innerHTML = `
        ...
        <div class="text-secondary" style="font-weight:600">Repos</div>
        <button class="btn btn-icon" onclick="openSettings()">${Icons.Gear}</button>
    `;
    ...
    nextSetInfo.textContent = `À venir : Série ${state.currentSet + 1} / ${state.targetSets}`;
...
function renderSettingsMode() {
    ...
    header.innerHTML = `
        <div></div>
        <h3>Réglages</h3>
        <button class="btn btn-secondary" style="padding: 8px 16px; height: 32px; font-size: 14px;" onclick="closeSettings()">OK</button>
    `;
    ...
    content.innerHTML = `
        <div style="margin-bottom: 20px;">
            <label class="text-label">Objectif de Séries</label>
            <input type="number" class="setting-input" value="${state.targetSets}" onchange="updateState('targetSets', this.value)">
        </div>
        <div style="margin-bottom: 20px;">
            <label class="text-label">Temps de Repos (sec)</label>
            <input type="number" class="setting-input" value="${state.restTimer}" onchange="updateState('restTimer', this.value)">
        </div>
        <div>
            <label class="text-label">Exercice</label>
            <div class="text-secondary" style="margin-top: 8px; font-size: 16px;">${state.currentExercise ? state.currentExercise.name : 'Aucun'}</div>
        </div>
    `;
...
function startRest() {
    ...
    state.timerInterval = setInterval(() => {
        ...
        if (state.timerRemaining <= 0) {
            ...
            // Update timer display directly to avoid full re-render flicker
...
function goBackToSelection() {
    if (confirm("Terminer la séance en cours ?")) {
        state.mode = 'SELECTION';
        render();
    }
}
